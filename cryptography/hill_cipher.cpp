...
void multiplyMatrices(int a[1000][3] , int a_rows , int a_cols , int b[1000][3] , int b_rows , int
b_cols , int res[1000][3])
{
...
void Inverse(int m[3][3] , int n , int m_inverse[3][3] )
{
int adj[3][3] = {0};
int det = Det(m , n);
int detInverse = DetInverse(det);
if(n==2)
{
adj[0][0] = m[1][1];
adj[1][1] = m[0][0];
...
(
pt.length())/n; // number of rows in P
for(int i=0; i<row ; i++)
{
for(int j=0; j<n; j++)
{
